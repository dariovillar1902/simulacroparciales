...
import bancoPreguntasFinalMetalicas from '../data/bancoPreguntasFinalMetalicas.json';
import { Button, Card } from 'react-bootstrap';

export const MetalicasFinalScreen = () => {
    ...
    const seleccionPregunta = useCallback(
        (id) => {
            let pregunta = bancoPreguntasFinalMetalicas.find(pregunta => pregunta.id === id);
            if (pregunta) pregunta.activa = 1;
            let filtroActivas = bancoPreguntasFinalMetalicas.filter(pregunta => pregunta.activa === 0);
