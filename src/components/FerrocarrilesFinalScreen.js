...
import bancoPreguntasFinalFerrocarriles from '../data/bancoPreguntasFinalFerrocarriles.json';
import { Button, Card } from 'react-bootstrap';

export const FerrocarrilesFinalScreen = () => {
    ...
    const seleccionPregunta = useCallback(
        (id) => {
            let pregunta = bancoPreguntasFinalFerrocarriles.find(pregunta => pregunta.id === id);
            if (pregunta) pregunta.activa = 1;
            let filtroActivas = bancoPreguntasFinalFerrocarriles.filter(pregunta => pregunta.activa === 0);
            ...
            setMostrarRespuesta(false); // Hide the answer when a new question is selected
